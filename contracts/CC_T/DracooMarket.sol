...
//重新包装一次erc20 用于防止可重入攻击|pankswap写法
/**
 * @title SafeERC20
...
contract DracooMarket is Context, Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    // owner MUST call Dracoo contract's "setApproveForAll" before use this method 放一部分nft出去售卖
    function offerNftForSale(address nftAddress, uint256 tokenId, address erc20Address, uint256 salePrice) public nonReentrant returns (uint256) {
        require(isMarketPaused == false, "market is paused now, try later");//没有开始
        require(_nftAvailable[nftAddress], "this Nft is not supported to sell here");//地址不具备资格
        require(_erc20Available[erc20Address], "This Token is NOT supported");//不具备erc20资格
        require(!_isNftOnSale[nftAddress][tokenId], "this nft's tokenId is already on sale now");//tokenid不能被打折出售
        require(salePrice > 0, "sale price must > 0");//打折的价格必须大于0
        IERC721 nftObject = IERC721(nftAddress); //地址对象
        require(nftObject.getApproved(tokenId) == address(this) || nftObject.isApprovedForAll(_msgSender(), address(this)), "not approved");//授权操作

        // start from #1
        _transactionId.increment();//tokenid 先增加
        uint256 currentId = _transactionId.current();//获取增加后的token-id
